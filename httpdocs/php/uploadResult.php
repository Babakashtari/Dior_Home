    ...
    function validator(){
        ...
        if(isset($_POST['submit'])){  
            ...
            if(!empty($_FILES['uploadingfile']) && !empty($_POST['file_name']) && !empty($_POST['category']) && !empty($_POST['subcategory'])){
                ...
                if(!empty($_POST['description']) && empty($text_area)){
                    echo '<p class="text-danger text-center pt-4 pb-1"><span class=" fas fa-exclamation-circle" aria-hidden="true"></span></p>';
                    echo '<p class="text-center text-danger error" >توضیحات سفارش به درستی وارد نشده است. </p>';
                }
                if(!is_uploaded_file($file_TMP_name)){
                    echo '<p class="text-danger text-center pt-4 pb-1"><span class=" fas fa-exclamation-circle" aria-hidden="true"></span></p>';
                    echo '<p class="text-center text-danger error" >فایلی برای آپلود انتخاب نشده است. </p>';
                }
                if(!empty($user_inserted_name) && !empty($category) && !empty($sub_category) && is_uploaded_file($file_TMP_name) && (empty($_POST['dimensions']) || (!empty($_POST['dimensions']) && !empty($file_dimension))) && (empty($_POST['description']) || (!empty($_POST['description']) && !empty($text_area)))){
